...
class MyGame(arcade.Window):
    ...
    def mujoco_thread(self, chain, D_Ori):
        ...
        for i in range(1000):
            ...
            while True:
                if self.demo_run:
                    if joint_node.new_callback_received:
                        # if joint_node.new_signal_received:  # just newly added, can delete it

                            joint_node.new_callback_received = False
                            # joint_node.new_signal_received = False

                            saved_position = joint_node.current_position  # just newly added, can delete it and switch back to joint_node.current_position
                            ...
                            self.current_error = np.array(joint_node.old_target_position) - np.array(
                                saved_position)

                            self.joint_node.target_position = np.array(alpha)

                            self.error_threshold_0 = abs(
                                (self.joint_node.target_position[0] - saved_position[0]) / 2)
                            self.error_threshold_1 = abs(
                                (self.joint_node.target_position[1] - saved_position[1]) / 2)
                            self.error_threshold_2 = abs(
                                (self.joint_node.target_position[2] - saved_position[2]) / 2)
                            self.error_threshold_3 = abs(
                                (self.joint_node.target_position[3] - saved_position[3]) / 2)
                            self.error_threshold_4 = abs(
                                (self.joint_node.target_position[4] - saved_position[4]) / 2)
                            self.error_threshold_5 = abs(
                                (self.joint_node.target_position[5] - saved_position[5]) / 2)
                            ...
                            # f = time.time()
                            if error_check:
                                # self.joint_node.send_clear_request()
                                # print("Error: ", self.current_error)
                                self.joint_node.send_positions(alpha)
                                self.joint_node.old_target_position = self.joint_node.target_position
                                # print("Test function executed")
