...
SCREEN_WIDTH = 1800 # 1200
SCREEN_HEIGHT = 1000  # 900
BUTTON_WIDTH = 250
BUTTON_HEIGHT = 50
square_size = 50  # Size of each square in the heatmap
BUTTON_X_POSITIONS = SCREEN_WIDTH / 10  # Adjust based on your layout
BUTTON_Y_POSITIONS = [900, 700, 630, 560, 490, 420, 350, 150]  # Adjust based on your layout and number of buttons
...
class MyGame(arcade.Window):
    def __init__(self, width, height, title, commander, ros_publisher):
        ...
        self.matrix_width = 0
        self.matrix_height_2 = 0
        self.matrix_width_2 = 0
        self.show_graph = False  # Add this line
        self.y_axis_center_value = None
        self.smoothed_curve_data = {}
        self.smoothed_curve_data_2 = {}

        self.initial_threshold_calculated = {}
        self.initial_threshold_calculated_2 = {}

        self.threshold_diff = {}
        self.threshold_diff_2 = {}
        self.threshold_max = {}
        self.threshold_max_2 = {}
        self.threshold_min = {}
        self.threshold_min_2 = {}
        self.filtered_data = {}
        self.filtered_data_2 = {}
        ...
        self.threshold_triggered_2 = {}  # To track if the threshold condition is met

        self.current_threshold_max = {}
        self.current_threshold_min = {}

        self.processed_data = {}  # Stores processed data for all cells
        self.processed_data_2 = {}  # Stores processed data for all cells

        self.temp_smoothed_data = {}
        self.temp_smoothed_data_2 = {}

        self.process_all = False
        self.initial_processing_done = False
        self.initial_processing_done_2 = False

        self.cells_processed_initially = set()
        self.cells_processed_initially_2 = set()
        self.red_intensity_dict = {}  # Initialize the dictionary to store red intensity values
        self.red_intensity_dict_2 = {}  # Initialize the dictionary to store red intensity values

        self.start_x = SCREEN_WIDTH / 10 - (square_size * self.matrix_height) / 10 + 200
        self.start_y = SCREEN_HEIGHT / 10 - (square_size * self.matrix_width) / 10 + 150
        self.start_x_2 = SCREEN_WIDTH / 10 - (square_size * self.matrix_height_2) / 10 + 1000
        self.start_y_2 = SCREEN_HEIGHT / 10 - (square_size * self.matrix_width_2) / 10 + 150
    ...
    def setup_buttons(self):
        ...
        button_commands = [
            ...
            ("Deactivation", self.deactivate_sensor_2),
    ...
    def perform_channel_check(self):
        # Get the channel check data, assuming it is a list of lists
        channel_check_data = self.commander.channel_check()

        # Dictionary to store results
        self.sensor_channel = {}

        # Iterate over the channel_check_data assuming it's a list of lists
        for index, data in enumerate(channel_check_data):
                self.sensor_channel[index] = {
                    'Channel Drive': data[0],
                    'Channel Sensor': data[1]
                }

        # Print the results
        # for idx, info in self.sensor_channel.items():
        #     print(f"Results for Sensor {idx}: {info}")

        # Access the 'Channel Drive' of the first index if it exists
        # if 0 in self.sensor_channel:
        #     first_index_drive = self.sensor_channel[0]['Channel Drive']
        #     second_index_drive = self.sensor_channel[0]['Channel Sensor']
        #     print("Channel Drive at first index:", first_index_drive)
        #     print("Channel Sensor at first index:", second_index_drive)

    def perform_read_raw(self):   # <-------------------------  haven't tested this function
        raw_data = self.commander.read_raw()

        # Assume that each list in raw_data needs to be handled separately
        for index, data_list in enumerate(raw_data):
            # Create a dictionary entry for each sensor reading set
            # You could categorize data based on the type or just index them if the type is not known
            raw_data[index] = data_list

        self.raw_data = raw_data[0]

        if len(raw_data) > 1:
            self.raw_data_2 = raw_data[1]
    ...
    def deactivate_sensor_2(self):
        self.deactivate = not self.deactivate
    ...
    def on_update(self, delta_time):
        ...
        if self.automation_active:
            ...
            if self.raw_data_2:
                # Update the heatmap data
                self.heatmap_data_2 = self.raw_data_2 # <---------- Test for raw data
        ...
        if self.selected_cell_2:
            row, col = self.selected_cell_2
            index = row * self.matrix_width + col
            if index < len(self.heatmap_data_2):
                print(f"Index: {index}, Row: {row}, Col: {col}, Value: {self.heatmap_data_2[index]}")

        if self.process_all:  # <-- GOGOGO
    ...
    def draw_heatmap(self):
        ...
        min_value = 0
        max_value = 100
        self.matrix_height = self.sensor_channel[0]['Channel Drive'] - 1   # Height of the matrix
        self.matrix_width = self.sensor_channel[0]['Channel Sensor']  # Width of the matrix
        ...
        for row in range(self.matrix_height):
            ...
            for col in range(self.matrix_width):
                ...
                x = self.start_x + row * square_size
                y = self.start_y + (self.matrix_width - 1 - col) * square_size
        ...
        sensor_1_signal = red_intensity_list.copy()  # dim: 110
        sensor_1_signal = sensor_1_signal[:100]  # dim: 100
        self.ros_publisher.publish_data(sensor_1_signal)
    ...
    def draw_heatmap_2(self):
        ...
        min_value = 0
        max_value = 100
        self.matrix_height_2 = self.sensor_channel[1]['Channel Drive'] - 1   # Height of the matrix
        self.matrix_width_2 = self.sensor_channel[1]['Channel Sensor']  # Width of the matrix

        for row_2 in range(self.matrix_height_2):
            if row_2 == useless_row:
                continue
            for col_2 in range(self.matrix_width_2):
                if col_2 == useless_col:
                    continue

                x = self.start_x_2 + row_2 * square_size
                y = self.start_y_2 + (self.matrix_width_2 - 1 - col_2) * square_size

                if (row_2, col_2) in self.smoothed_curve_data_2 and self.smoothed_curve_data_2[(row_2, col_2)]:
                    value = self.smoothed_curve_data_2[(row_2, col_2)][-1]
                else:
                    value = 0

                if self.selected_cell_2 == (row_2, col_2):
                    color = arcade.color.BLUE
                elif self.threshold_triggered_2.get((row_2, col_2), False) is True:
                    difference = self.threshold_min_2[(row_2, col_2)] - value
                    # Scale the difference to a value between 0 and 255, considering the range of difference
                    red_intensity = int(((difference - min_value) / (max_value - min_value)) * 255)
                    red_intensity = max(0, min(255, red_intensity))  # Ensure red_intensity is within 0-255
                    self.red_intensity_dict_2[(row_2, col_2)] = red_intensity  # Store the intensity in the dictionary
                    color = (255, 255 - red_intensity, 255 - red_intensity)
                else:
                    color = arcade.color.LIGHT_GRAY
                    self.red_intensity_dict_2[(row_2, col_2)] = 0  # Store 0 intensity for cells not meeting threshold

                arcade.draw_rectangle_filled(x, y, square_size, square_size, color)
                text_color = arcade.color.BLACK
                # arcade.draw_text(str(self.red_intensity_dict[(row, col)]), x, y, text_color, 12, width=square_size, align="center", anchor_x="center",
                #                  anchor_y="center")
                # arcade.draw_text(f"{row_2}, {col_2}", x, y, text_color, 12, width=square_size, align="center", anchor_x="center",
                #                  anchor_y="center")

        # Assuming you want to use 0 as the default red intensity for missing cells
        red_intensity_list = [self.red_intensity_dict_2.get((row, col), 0) for row in range(self.matrix_height_2) for col in
                              range(self.matrix_width_2) if
                              (row, col) in self.red_intensity_dict_2 or (row != useless_row and col != useless_col)]

        sensor_2_signal = red_intensity_list.copy()  # dim: 110
        sensor_2_signal = sensor_2_signal[:100]  # dim: 100
        self.ros_publisher.publish_data_2(sensor_2_signal)
    ...
    def process_all_cells(self):
        ...
        if not self.processed_data_2:  # Initialize if empty
            for row in range(self.matrix_height_2):
                for col in range(self.matrix_width_2):
                    self.processed_data_2[(row, col)] = []
            print(f"Processed data initialized with {self.matrix_height_2} x {self.matrix_width_2} cells.")
            print(f"Length of processed data: {len(self.processed_data_2)}")
        ...
        if self.deactivate:
            if self.processed_data_2:
                for row in range(self.matrix_height_2):
                    for col in range(self.matrix_width_2):
                        cell_index = row * self.matrix_width_2 + col  # matrix_width is 10, because 10 columns
                        if cell_index < len(self.heatmap_data_2):  # Check if the index is within the data range, which is 110
                            self.process_cell_data_2(row, col, self.heatmap_data_2[cell_index])  # Process the data for each cell repeatedly
                        else:
                            print(f"Data for cell ({row}, {col}) is out of range and cannot be processed.")
    ...
    def process_cell_data_2(self, row, col, data):  # Only 1 cell of data is entering here each time
        """Append data for a single cell identified by row and col."""
        if self.deactivate:

            self.processed_data_2[(row, col)].append(data)

            if len(self.processed_data_2[(row, col)]) > number_of_data:
                self.processed_data_2[(row, col)].pop(0)
            if len(self.processed_data_2[(row, col)]) == number_of_data:
                self.set_thresholds_2(row, col, self.processed_data_2[(row, col)])
                # Add cell to the tracker
                self.cells_processed_initially_2.add((row, col))

                # Check if all cells have reached the initial processing stage
                if len(self.cells_processed_initially_2) == self.matrix_height_2 * self.matrix_width_2 and not self.initial_processing_done_2:
                    print("Start processing the data")
                    self.initial_processing_done_2 = True
    ...
    def set_thresholds_2(self, row, col, data):  # "number_of_data" data of 1 cell is entering here
        # Calculate IQR for outlier detection
        if self.deactivate:
            ...
            # Filter out outliers
            self.filtered_data_2[(row, col)] = data

            # Update the smoothed curve with filtered data
            self.smoothed_curve_data_2[(row, col)] = self.calculate_moving_average_2(row, col, self.filtered_data_2[(row, col)])

            # Calculate the initial threshold
            if (row, col) not in self.threshold_max_2 or not self.initial_threshold_calculated_2[(row, col)]:
                self.threshold_max_2[(row, col)] = max(self.filtered_data_2[(row, col)]) + threshold_offset
                self.threshold_min_2[(row, col)] = min(self.filtered_data_2[(row, col)]) - threshold_offset
                self.threshold_diff_2[(row, col)] = self.threshold_max_2[(row, col)] - self.threshold_min_2[(row, col)]

                # Set flag that initial thresholds are calculated
                self.initial_threshold_calculated_2[(row, col)] = True
    ...
    def recalculate_thresholds_for_all_cells_2(self):
        if self.deactivate:

            for row in range(self.matrix_height_2):
                for col in range(self.matrix_width_2):
                    cell_key = (row, col)
                    if cell_key in self.processed_data_2:
                        # Recalculate thresholds using existing processed data for the cell
                        data = self.processed_data_2[cell_key]
                        if data:  # Ensure there is data to process
                            self.initial_threshold_calculated_2[(row, col)] = False  # Reset the flag
                            self.set_thresholds_2(row, col, data)
    ...
    def calculate_moving_average_2(self, row, col, data, window_size=10):   # "number_of_data - filtered out data" of 1 cell is entering here
        if self.deactivate:

            # Calculate the moving average for the given data
            if (row, col) not in self.temp_smoothed_data_2:
                self.temp_smoothed_data_2[(row, col)] = []
            ...
            # Assign the calculated moving averages directly to ensure the lengths match
            self.temp_smoothed_data_2[(row, col)] = temp_smoothed_data

            # Return the updated smoothed data for this cell
            return self.temp_smoothed_data_2[(row, col)]

    def check_heatmap_click(self, x, y):
        for row in range(self.matrix_height):
            for col in range(self.matrix_width):
                # Adjust for the 180-degree rotation, similar to the adjustments in draw_heatmap
                cell_x = self.start_x + row * square_size - square_size / 2  # Center of cell horizontally
                cell_y = self.start_y + (self.matrix_width - 1 - col) * square_size - square_size / 2  # Center of cell vertically
                ...
                if (cell_x <= x <= cell_x + square_size and
                    ...
                    if self.selected_cell == (row, col):
                        ...
                        self.selected_cell_2 = None  # Deselect the second heatmap cell
                    return

        for row_2 in range(self.matrix_height_2):
            for col_2 in range(self.matrix_width_2):
                # Adjust for the 180-degree rotation, similar to the adjustments in draw_heatmap
                cell_x_2 = self.start_x_2 + row_2 * square_size - square_size / 2  # Center of cell horizontally
                cell_y_2 = self.start_y_2 + (self.matrix_width_2 - 1 - col_2) * square_size - square_size / 2  # Center of cell vertically
                # Check if the click is within the bounds of the cell
                if cell_x_2 <= x <= cell_x_2 + square_size and cell_y_2 <= y <= cell_y_2 + square_size:
                    # Toggle selection state for the clicked cell
                    if self.selected_cell_2 == (row_2, col_2):
                        self.selected_cell_2 = None  # Deselect if already selected
                    else:
                        self.selected_cell_2 = (row_2, col_2)  # Select the cell
                        self.selected_cell = None  # Deselect the second heatmap cell
                    return
    ...
    def check_threshold_condition_2(self):
        if self.deactivate:
            ...
            for row in range(self.matrix_height_2):
                for col in range(self.matrix_width_2):
                    ...
                    # Assume the cell has not triggered the threshold initially
                    self.threshold_triggered_2[cell_key] = False

                    if cell_key in self.smoothed_curve_data_2 and len(self.smoothed_curve_data_2[cell_key]) >= 2:
                        last_data = self.smoothed_curve_data_2[cell_key][-2:]
                        above_threshold = all(d > self.threshold_max_2[cell_key] for d in last_data)
                        below_threshold = all(d < self.threshold_min_2[cell_key] for d in last_data)

                        if above_threshold:
                            self.threshold_triggered_2[cell_key] = True
                            above_threshold_triggered = True
                        elif below_threshold:
                            self.threshold_triggered_2[cell_key] = True
                            below_threshold_triggered = True
    ...
    def update_background_color_for_selected_cell(self):
        ...
        if self.deactivate:
            if self.selected_cell_2:  # Ensure there is a selected cell
                cell_key = self.selected_cell_2
                # Check if the selected cell has triggered the threshold
                if cell_key in self.threshold_triggered_2 and self.threshold_triggered_2[cell_key]:
                    # Check whether it's above or below the threshold
                    if len(self.smoothed_curve_data_2[cell_key]) >= 2:
                        last_data = self.smoothed_curve_data_2[cell_key][-2:]
                        above_threshold = all(d > self.threshold_max_2[cell_key] for d in last_data)
                        below_threshold = all(d < self.threshold_min_2[cell_key] for d in last_data)
    ...
    def on_draw(self):
        ...
        if self.deactivate:
            self.draw_realtime_curve_2()
            self.check_threshold_condition_2()
    ...
    def on_mouse_press(self, x, y, button, modifiers):
        ...
        # Right click
            ...
            if self.show_graph and self.selected_cell_2:
                row, col = self.selected_cell_2
                index = row * self.matrix_width_2 + col
                if index < len(self.heatmap_data_2):
                    self.y_axis_center_value = self.heatmap_data_2[index]
    ...
    def draw_realtime_curve_2(self):
        if self.deactivate:

            if not self.selected_cell_2 or not self.processed_data_2 or not self.show_graph:
                return
            ...
            if self.y_axis_center_value is not None:
                ...
                y_min = min(self.processed_data_2[(row, col)], default=0) - 100
                y_max = max(self.processed_data_2[(row, col)], default=0) + 100

            x_scale = graph_width / len(self.processed_data_2[(row, col)])
            y_scale = graph_height / (y_max - y_min)

            # Draw the graph lines
            for i in range(1, len(self.processed_data_2[(row, col)])):
                x1 = graph_left + (i - 1) * x_scale
                y1 = graph_top - graph_height + (self.processed_data_2[(row, col)][i - 1] - y_min) * y_scale
                x2 = graph_left + i * x_scale
                y2 = graph_top - graph_height + (self.processed_data_2[(row, col)][i] - y_min) * y_scale
                arcade.draw_line(x1, y1, x2, y2, arcade.color.BLUE, 2)

            if self.smoothed_curve_data_2[(row, col)] and self.initial_threshold_calculated_2:

                # Draw the threshold lines
                self.draw_threshold_line(self.threshold_max_2[(row, col)], arcade.color.BROWN, y_min, y_max, y_scale, graph_top, graph_height, graph_left, graph_width)
                self.draw_threshold_line(self.threshold_min_2[(row, col)], arcade.color.BROWN, y_min, y_max, y_scale, graph_top, graph_height, graph_left, graph_width)

            # Draw original and smoothed curves
            self.draw_curve(self.processed_data_2[(row, col)], arcade.color.BLUE, y_min, y_max, y_scale, graph_left, graph_top,
                            graph_height, graph_width)
            self.draw_curve(self.smoothed_curve_data_2[(row, col)], arcade.color.RED, y_min, y_max, y_scale, graph_left, graph_top,
                            graph_height, graph_width)
