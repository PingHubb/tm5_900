    ...
    with open(filename, 'r') as file:
        for line in file:
            ...
            if line.startswith('---'):  # Identify a new step
                ...
                continue
            numbers = line.split(', ')
            if len(numbers) > 22:  # Ensure there are enough elements
                current_step_data.append(float(numbers[22]))  # Convert to float and add to list
...
def plot_data(data):
    ...
    plt.title('Sensor Data at C Across Different Steps')
    plt.xlabel('Time')
    plt.ylabel('Raw data value at C')
    plt.legend()
    plt.grid(True)
    plt.show()
...
if __name__ == '__main__':
    filename = '/home/ping2/ros2_ws/src/lan_control/sensor_readings_C.txt'  # Update with your actual file path
